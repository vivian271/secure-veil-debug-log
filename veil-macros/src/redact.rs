...
use syn::{parse::Parse, spanned::Spanned, LitFloat};
...
/// Parses the `noise_level` attribute which specifies the amount of noise to add.
fn parse_noise_level(attr: &syn::Attribute) -> Result<f64, syn::Error> {
    attr.parse_args::<LitFloat>()
        .map(|lit| lit.base10_parse::<f64>().unwrap_or(0.0))
        .map_err(|_| syn::Error::new(attr.span(), "Expected a floating point number for `noise_level`"))
}
